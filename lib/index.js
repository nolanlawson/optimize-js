...
function optimizeJs (jsString, opts) {
  ...
  function handleFunctionExpression (node) {
    ...
    // console('nother function')
    // console(node.id && node.id.name)
    // console('parnet node type', node.parentNode.type)
    // console(node.parentNode)
    // console(node.parentNode.callee === node)
    if (preChar === '!' && postChar === '(') {
      // console('uglify-style')
